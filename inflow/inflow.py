...
def generate_inflow_file(input_file_directory, weight_table_file=None,
    ...
    input_file_expr = os.path.join(input_file_directory, f'*.{input_file_ext}')
    ...
    lsm_timestamp_list = [parse_timestamp_from_filename(
        f, file_timestamp_re_pattern, file_datetime_format) for f in
                      lsm_file_list]
    lsm_timestamp_array = np.array(timestamp_list)
    sorted_by_time = timestamp_array.argsort()
    lsm_file_array = lsm_file_array[sorted_by_time]
    lsm_timestamp_array = timestamp_array[sorted_by_time]

    in_time_bounds = np.ones_like(timestamp_array, dtype=bool)
    
    if simulation_start_datetime is not None:
        on_or_after_start = timestamp_array >= simulation_start_datetime
        in_time_bounds = np.logical_and(in_time_bounds, on_or_after_start)

    if simulation_end_datetime is not None:
        on_or_before_end = timestamp_array <= simulation_end_datetime
        in_time_bounds = np.logical_and(in_time_bounds, on_or_before_end)

    lsm_file_array = lsm_file_array[in_time_bounds]
    timestamp_array = timestamp_array[in_time_bounds]
    
    return (lsm_file_array, timestamp_array)

if __name__=='__main__':
    input_file_directory = 'tests/data/lsm_grids/gldas2'
    file_datetime_format = '%Y%m%d.%H'
    file_timestamp_re_pattern = r'\d{8}.\d{2}'
    simulation_start_datetime = datetime(2010, 12, 31, 3)
    simulation_end_datetime = datetime(2010, 12, 31)
    flist, tlist = generate_inflow_file(input_file_directory,
                                        input_file_ext='nc4',
